...
const placeData = {
    "Hong Kong": {
        img: testImage,
        title: "Country",
        description: "An amazing city with the perfect combination of east and west. With a population of 7.4 million the city is always busting with noise.",
    },
    "Mangere":{
        img: testImage,
        title: "Country",
    ...
    "Paris":{
        ...
        title: "Country",
        description:
            "In this world full of sadness, all we really need is someone to love. Maybe you'll find it here.",
    },
};
...
class PopularView extends React.Component{
    ...
    constructor(props){
        ...
        this.state = {
            places: [],
            placeIDs: ["ChIJBxpST4VPDW0R0N6iQ2HvAAU",
    ...
    componentDidMount(){
        const getPlaceDetails = async() =>{
            ...
            this.state.placeIDs.map((placeID, index) =>
                {try{
                    axios
    ...
    render(){
        ...
        return (
            <div className={classes.root}>
                <Typography className={classes.title} variant="h3">{"Recommended Places"}</Typography>
                {this.state.places.map((place, index) => (
                    <Card className={classes.card} key={index}>
                        <CardActionArea className={classes.cardActionArea}>
                            <CardMedia className={classes.media} image={`${GOOGLE_PLACES_API_URL}/photo?maxwidth=400&photoreference=${place.photoReference}&key=${GOOGLE_PLACES_API_KEY}`}/>
